...
async function startServer() {
  try {
    ...
    const server = app.listen(env.PORT, () => {
      logger.info('============================================');
      logger.info('  DocBridge Health Summary Service');
    ...
    function gracefulShutdown(signal) {
      logger.info(`${signal} received. Shutting down health-summary-service gracefully...`);
      server.close(async () => {
        try {
    ...
    logger.error('Failed to start health-summary-service:', error.message);
    process.exit(1);
  }
}
