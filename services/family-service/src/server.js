...
async function startServer() {
  try {
    ...
    const server = app.listen(env.PORT, '127.0.0.1', () => {
      logger.info('============================================');
      logger.info('  DocBridge Family Service');
    ...
    function gracefulShutdown(signal) {
      logger.info(`${signal} received. Shutting down family-service gracefully...`);
      server.close(async () => {
        try {
    ...
    logger.error('Failed to start family-service:', error.message);
    process.exit(1);
  }
}
