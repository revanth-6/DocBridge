...
export default function PrescriptionsPage() {
  ...
  const fetchData = () => {
    setLoading(true);
    prescriptionApi.list({ search, page: 1, limit: 20 })
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      {items.length === 0 ? (
        <EmptyState icon="📋" title="No prescriptions yet" description="Start by adding your first entry." action={<Button onClick={() => setShowCreate(true)}>Add Prescription</Button>} />
      ) : (
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Add Prescription">
