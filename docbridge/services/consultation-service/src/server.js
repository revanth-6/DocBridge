...
async function startServer() {
  try {
    ...
    const server = app.listen(env.PORT, () => {
      logger.info('============================================');
      logger.info('  DocBridge Consultation Service');
    ...
    function gracefulShutdown(signal) {
      logger.info(`${signal} received. Shutting down consultation-service gracefully...`);
      server.close(async () => {
        try {
    ...
    logger.error('Failed to start consultation-service:', error.message);
    process.exit(1);
  }
}
