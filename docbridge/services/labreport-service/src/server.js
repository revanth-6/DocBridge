...
async function startServer() {
  try {
    ...
    const server = app.listen(env.PORT, () => {
      logger.info('============================================');
      logger.info('  DocBridge Lab Report Service');
    ...
    function gracefulShutdown(signal) {
      logger.info(`${signal} received. Shutting down labreport-service gracefully...`);
      server.close(async () => {
        try {
    ...
    logger.error('Failed to start labreport-service:', error.message);
    process.exit(1);
  }
}
